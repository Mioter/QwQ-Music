...
public class Filter : SoundComponent
{
    ...
    private void CalculateCoefficients()
    {
        ...
        float omega = 2.0f * MathF.PI * CutoffFrequency / sampleRate; // Angular frequency
        float sinOmega = MathF.Sin(omega);
        float cosOmega = MathF.Cos(omega);
        float alpha = sinOmega / (2 * Resonance); // Bandwidth parameter, related to resonance
