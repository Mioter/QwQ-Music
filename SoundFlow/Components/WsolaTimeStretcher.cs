...
public class WsolaTimeStretcher
{
    ...
    public void SetSpeed(float speed)
    {
        ...
        _nominalHopSynthesisFrames = (int)Math.Max(1, Math.Round(NOMINAL_ANALYSIS_HOP_FRAMES / _speed));
    }
    ...
    public int MinInputSamplesToProcess =>
        (NOMINAL_ANALYSIS_HOP_FRAMES + SEARCH_RADIUS_FRAMES) * _channels + _windowSizeSamples;
    ...
    )
    {
        ...
        if (!input.IsEmpty)
        {
            if (_inputBufferReadPos > 0 && _inputBufferValidSamples > _inputBufferReadPos)
                Buffer.BlockCopy(
                    _inputBufferInternal,
                    _inputBufferReadPos * sizeof(float),
                    _inputBufferInternal,
                    0,
            ...
            _inputBufferReadPos = 0;
            int spaceInInputBuffer = _inputBufferInternal.Length - _inputBufferValidSamples;
        ...
        int totalSourceSamplesForThisCall = 0;
        ...
        while (samplesWrittenToOutput < output.Length)
        {
            ...
            int bestOffsetFromNominalFrames = 0;
            ...
            {
                int synthesisHopSamples = _nominalHopSynthesisFrames * _channels;
                // Length of the overlap region to compare.
                int compareLengthSamples = Math.Min(_actualPrevTailLength, _windowSizeSamples - synthesisHopSamples);
                compareLengthSamples = Math.Max(0, compareLengthSamples);
                int compareLengthFrames = compareLengthSamples / _channels;
                const int minValidOverlapForSearch = SEARCH_RADIUS_FRAMES / 4;
                float prevTailEnergy = 0;
                if (compareLengthFrames > 0)
                {
                    for (int iS = 0; iS < compareLengthSamples; ++iS)
                        prevTailEnergy += _prevOutputTail[iS] * _prevOutputTail[iS];
                }

                float silenceThreshold = 1e-7f * compareLengthSamples; // Threshold to avoid correlation on silence.

                // Only perform search if previous tail has significant energy and enough length for meaningful correlation.
                if (
                    prevTailEnergy > silenceThreshold
                    && compareLengthFrames > minValidOverlapForSearch
                ...
                )
                {
                    ...
                    double sumA = 0;
                    for (int iS = 0; iS < compareLengthSamples; ++iS)
                        sumA += _prevOutputTail[iS];
                    double meanA = sumA / compareLengthSamples;
                    double sumADevSq = 0;
                    for (int iS = 0; iS < compareLengthSamples; ++iS)
                    {
                        double d = _prevOutputTail[iS] - meanA;
                        sumADevSq += d * d;
                    }

                    // Pre-calculate NCC for delta 0 (nominal hop) as a baseline.
                    int candidateStartAtDelta0 = _inputBufferReadPos + NOMINAL_ANALYSIS_HOP_FRAMES * _channels;
                    if (
                        NOMINAL_ANALYSIS_HOP_FRAMES > 0
                    ...
                    )
                    {
                        ...
                        double meanBd0 = sumBd0 / compareLengthSamples;
                        double sumBDevSqD0 = 0,
                            dotProductDevD0 = 0;
                        for (int iS = 0; iS < compareLengthSamples; ++iS)
                        {
                            double dA = _prevOutputTail[iS] - meanA;
                            double dB = _inputBufferInternal[candidateStartAtDelta0 + iS] - meanBd0;
                            dotProductDevD0 += dA * dB;
                            sumBDevSqD0 += dB * dB;
                        }

                        double denominatorD0 = Math.Sqrt(sumADevSq * sumBDevSqD0);
                        if (denominatorD0 < 1e-9)
                            maxNcc = sumADevSq < 1e-9 && sumBDevSqD0 < 1e-9 ? 1.0 : 0.0;
                        else
                            maxNcc = dotProductDevD0 / denominatorD0;
                    }

                    // Iterate through search radius to find the best overlap.
                    for (
                        int currentDeltaFrames = -SEARCH_RADIUS_FRAMES;
                        currentDeltaFrames <= SEARCH_RADIUS_FRAMES;
                        currentDeltaFrames++
                    )
                    {
                        if (currentDeltaFrames == 0)
                            continue;
                        int trialAnalysisHopFrames = NOMINAL_ANALYSIS_HOP_FRAMES + currentDeltaFrames;
                        if (trialAnalysisHopFrames <= 0)
                            continue;
                        int candidateSegmentStartSample = _inputBufferReadPos + trialAnalysisHopFrames * _channels;
                        ...
                        double meanB = sumB / compareLengthSamples;
                        double sumBDevSq = 0,
                            dotProductDev = 0;
                        for (int iS = 0; iS < compareLengthSamples; ++iS)
                        {
                            double dA = _prevOutputTail[iS] - meanA;
                            double dB = _inputBufferInternal[candidateSegmentStartSample + iS] - meanB;
                            dotProductDev += dA * dB;
                            sumBDevSq += dB * dB;
                        }
                        ...
                        double currentNcc;
                        double denominator = Math.Sqrt(sumADevSq * sumBDevSq);
                        if (denominator < 1e-9)
            ...
            int actualAnalysisHopFrames = NOMINAL_ANALYSIS_HOP_FRAMES + bestOffsetFromNominalFrames;
            if (actualAnalysisHopFrames <= 0)
                actualAnalysisHopFrames = 1;
            int actualAnalysisHopSamples = actualAnalysisHopFrames * _channels;

            // Calculate the starting position of the chosen analysis segment in the input buffer.
            int chosenSegmentStartSampleInInput =
                _inputBufferReadPos + (NOMINAL_ANALYSIS_HOP_FRAMES + bestOffsetFromNominalFrames) * _channels;
            ...
            for (int f = 0; f < DefaultWindowSizeFrames; f++)
            {
                ...
                {
                    int readIdx = chosenSegmentStartSampleInInput + f * _channels + ch;
                    _currentAnalysisFrame[f * _channels + ch] = _inputBufferInternal[readIdx] * _analysisWindow[f];
                }
            }

            // Perform overlap-add synthesis.
            int currentFrameSynthesisHopSamples = _nominalHopSynthesisFrames * _channels;
            int currentFrameSynthesisOverlapSamples = Math.Max(0, _windowSizeSamples - currentFrameSynthesisHopSamples);
            ...
            if (!_isFirstFrame && _actualPrevTailLength > 0)
            {
                int overlapToUseFromPrev = Math.Min(_actualPrevTailLength, currentFrameSynthesisOverlapSamples);
                if (overlapToUseFromPrev > 0)
                    _prevOutputTail
                        .AsSpan(0, overlapToUseFromPrev)
            ...
            for (int i = 0; i < _windowSizeSamples; ++i)
            {
                if (
                    !_isFirstFrame
                    && _actualPrevTailLength > 0
            ...
            int availableInOutputSpan = output.Length - samplesWrittenToOutput;
            int actualCopyToOutput = Math.Min(currentFrameSynthesisHopSamples, availableInOutputSpan);
            ...
            if (actualCopyToOutput > 0)
            {
                ...
                totalSourceSamplesForThisCall +=
                    _nominalHopSynthesisFrames > 0
                        ? (int)
                            Math.Round(
                                (double)actualCopyToOutput
                                    / (_nominalHopSynthesisFrames * _channels)
            ...
            {
                case > 0 when _prevOutputTail.Length >= currentFrameSynthesisOverlapSamples:
                    Array.Copy(
                        _outputOverlapBuffer,
                        currentFrameSynthesisHopSamples,
                        _prevOutputTail,
                        0,
            ...
            if (samplesWrittenToOutput >= output.Length)
                break;
            if (_isFlushing && _inputBufferValidSamples - _inputBufferReadPos < _channels)
                break;
        }

        // Shift any remaining data in the internal input buffer to the beginning.
        int remainingInternalInput = _inputBufferValidSamples - _inputBufferReadPos;
