...
public class AudioSegment : IDisposable
{
    ...
    {
        SourceDataProvider = sourceDataProvider ?? throw new ArgumentNullException(nameof(sourceDataProvider));
        ...
        if (_sourceStartTime < TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(sourceStartTime));
        if (_sourceDuration <= TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(sourceDuration));
        if (_timelineStartTime < TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(timelineStartTime));
    ...
    {
        int channels = AudioEngine.Channels > 0 ? AudioEngine.Channels : 2;
        int sourceSampleRate = SourceDataProvider.SampleRate;
        ...
        if (SourceDataProvider.CanSeek)
            SourceDataProvider.Seek((int)_currentSourceDataProviderPhysicalReadPos);
        ...
        float effectiveStretchFactor = Settings.TimeStretchFactor;
    ...
    /// </summary>
    public TimeSpan StretchedSourceDuration =>
    ...
    /// </summary>
    public TimeSpan EffectiveDurationOnTimeline =>
    ...
    public TimeSpan GetTotalLoopedDurationOnTimeline()
    {
        ...
        if (singleInstanceDuration <= TimeSpan.Zero)
    ...
    public AudioSegment Clone(TimeSpan? newTimelineStartTime = null)
    {
        return new AudioSegment(
            ...
            SourceStartTime,
            SourceDuration,
            newTimelineStartTime ?? TimelineStartTime,
            $"{Name} (Clone)",
            Settings.Clone()
        );
    }
    ...
    public int ReadProcessedSamples(
        TimeSpan segmentTimelineOffset,
        TimeSpan durationToRead,
    ...
    )
    {
        ...
        int currentSegmentLoopPass =
            singleStretchedInstanceDuration.Ticks > 0
                ? (int)(timeIntoTotalLoopedStretchedContent.Ticks / singleStretchedInstanceDuration.Ticks)
                : 0;
        var timeOffsetWithinCurrentStretchedPass = TimeSpan.FromTicks(
            timeIntoTotalLoopedStretchedContent.Ticks % singleStretchedInstanceDuration.Ticks
        );

        if (
            !IsSegmentLoopEffectivelyInfinite(currentSegmentLoopPass)
            || Settings.Loop.TargetDuration.HasValue && segmentTimelineOffset >= Settings.Loop.TargetDuration.Value
        )
        {
            if (outputBufferOffset < outputBuffer.Length)
                outputBuffer[outputBufferOffset..].Clear();
        ...
        int sampleRateForStretchedOutput = SourceDataProvider.SampleRate;
        var stretchedDurationToFetch = TimeSpan.FromTicks((long)(durationToRead.Ticks * Settings.SpeedFactor));
        int samplesToFetchFromStretched = (int)(
            stretchedDurationToFetch.TotalSeconds * sampleRateForStretchedOutput * targetChannels
        );
        ...
        try
        {
            ...
            int finalOutputSamplesCount = (int)(durationToRead.TotalSeconds * targetSampleRate * targetChannels);
            ...
            if (
                Math.Abs(Settings.SpeedFactor - 1.0f) > float.Epsilon
                || sampleRateForStretchedOutput != targetSampleRate
            )
            {
                if (finalOutputSamplesCount > 0)
                {
                    ...
                    SimpleResample(
                        samplesToProcess,
                        resampledBuffer.AsSpan(0, finalOutputSamplesCount),
                        targetChannels
                    );
            ...
            int samplesWrittenToOutput = 0;
            var singleEffectiveInstanceDurationOnTimeline = EffectiveDurationOnTimeline;

            for (int frameIndex = 0; frameIndex < samplesToProcess.Length / targetChannels; frameIndex++)
            {
                // Ensure there's enough space in the output buffer for the current frame.
                if (outputBufferOffset + samplesWrittenToOutput + targetChannels > outputBuffer.Length)
                    break;
                ...
                var timeIntoCurrentEffectiveLoopInstance =
                    MapStretchedOffsetToEffectiveTimelineOffset(
                        timeOffsetWithinCurrentStretchedPass,
                ...
                {
                    float sample = processingFrameSpan[ch];
                    float fadeMultiplier = 1.0f;

                    // Apply Fade In
                    if (
                        Settings.FadeInDuration > TimeSpan.Zero
                        && timeIntoCurrentEffectiveLoopInstance < Settings.FadeInDuration
                    )
                        fadeMultiplier *= GetFadeMultiplier(
                            timeIntoCurrentEffectiveLoopInstance.TotalSeconds / Settings.FadeInDuration.TotalSeconds,
                    ...
                    // Apply Fade Out
                    if (
                        Settings.FadeOutDuration > TimeSpan.Zero
                        && timeIntoCurrentEffectiveLoopInstance
                            > singleEffectiveInstanceDurationOnTimeline - Settings.FadeOutDuration
                    )
                    {
                        double timeIntoFadeOut = (
                            timeIntoCurrentEffectiveLoopInstance
                            - (singleEffectiveInstanceDurationOnTimeline - Settings.FadeOutDuration)
                        ).TotalSeconds;
                        double fadeOutProgress = Math.Clamp(
                            timeIntoFadeOut / Settings.FadeOutDuration.TotalSeconds,
                            0.0,
                            1.0
                        );
                    ...
                    if (targetChannels == 2)
                    {
                        float panFactor = (Settings.Pan + 1.0f) / 2.0f;
                        sample *=
                            ch == 0
            ...
            if (
                outputBufferOffset + samplesWrittenToOutput < outputBuffer.Length
        ...
        finally
        {
            ...
            if (stretchedSamplesBuffer != null)
                ArrayPool<float>.Shared.Return(stretchedSamplesBuffer, true);
            if (resampledBuffer != null)
                ArrayPool<float>.Shared.Return(resampledBuffer, true);
            if (tempProcessingFrame != null)
                ArrayPool<float>.Shared.Return(tempProcessingFrame, true);
        }
    }
    ...
    private int GetTimeStretchedSamples(
        TimeSpan timeOffsetInCurrentStretchedPass,
        TimeSpan stretchedDurationToRead,
        Span<float> outputBuffer,
        int sampleRate,
        int channels,
        int currentSegmentLoopPass
    )
    {
        ...
        {
            return ReadAndReverseSourceSamples(
                timeOffsetInCurrentStretchedPass,
                stretchedDurationToRead,
                outputBuffer,
                sampleRate,
                channels,
                currentSegmentLoopPass,
                isFeedingWsola: false
            );
        }

        // WSOLA Path
        int samplesNeededForRequest = (int)(stretchedDurationToRead.TotalSeconds * sampleRate * channels);
        samplesNeededForRequest = Math.Min(samplesNeededForRequest, outputBuffer.Length);
        if (samplesNeededForRequest <= 0)
            return 0;

        int samplesFilledForRequest = 0;

        // Calculate target start sample within the stretched segment loop.
        long targetStartSampleInStretchedStreamThisSegmentLoop = (long)(
            timeOffsetInCurrentStretchedPass.TotalSeconds * sampleRate * channels
        );

        // Determine how many samples need to be skipped from the WSOLA output buffer to reach the target start.
        long samplesToSkipFromWsolaOutputBuffer =
            targetStartSampleInStretchedStreamThisSegmentLoop - _currentStretchedPlayheadInSegmentLoopSamples;

        // Skip samples that are already available in the WSOLA output buffer.
        long canSkipFromCurrentBuffer = Math.Min(
            samplesToSkipFromWsolaOutputBuffer,
        ...
        while (samplesFilledForRequest < samplesNeededForRequest)
        {
            ...
            while (samplesToSkipFromWsolaOutputBuffer > 0)
            {
                ...
                canSkipFromCurrentBuffer = Math.Min(
                    samplesToSkipFromWsolaOutputBuffer,
                    _wsolaOutputBufferValidSamples - _wsolaOutputBufferReadOffset
                );
                _wsolaOutputBufferReadOffset += (int)canSkipFromCurrentBuffer;
            ...
            int availableInWsolaOutput = _wsolaOutputBufferValidSamples - _wsolaOutputBufferReadOffset;
            int toCopyNow = Math.Min(samplesNeededForRequest - samplesFilledForRequest, availableInWsolaOutput);
    ...
    {
        if (_wsolaOutputBufferReadOffset <= 0)
            return;
        if (_wsolaOutputBufferValidSamples > _wsolaOutputBufferReadOffset)
        {
            ...
            Buffer.BlockCopy(
                _wsolaOutputBuffer,
                _wsolaOutputBufferReadOffset * sizeof(float),
                _wsolaOutputBuffer,
                0,
    ...
    private bool EnsureMoreWsolaOutputGenerated(int sampleRate, int channels)
    {
        ...
        long sourceSamplesInOneSourcePass = (long)(SourceDuration.TotalSeconds * sampleRate * channels);
        if (sourceSamplesInOneSourcePass <= 0)
            return false;

        int initialWsolaOutputCountThisCall = _wsolaOutputBufferValidSamples;
        ...
        )
        {
            bool currentSourcePassExhaustedForWsolaFeed =
                _sourceSamplesFedToWsolaThisSourcePass >= sourceSamplesInOneSourcePass;
            ...
            {
                int spaceToFillInFeed = _wsolaFeedBuffer.Length - _wsolaFeedBufferValidSamples;
                if (spaceToFillInFeed > 0)
                {
                    // Calculate how many samples are left in the current source pass.
                    long remainingInSourcePassForFeed =
                        sourceSamplesInOneSourcePass - _sourceSamplesFedToWsolaThisSourcePass;
                    int samplesToReadForFeed = Math.Min(spaceToFillInFeed, (int)remainingInSourcePassForFeed);
                    var sourceDurationToReadForFeed = TimeSpan.FromSeconds(
                        samplesToReadForFeed / (double)(sampleRate * channels)
                    );
                    var timeOffsetInCurrentSourcePassForFeed = TimeSpan.FromSeconds(
                        _sourceSamplesFedToWsolaThisSourcePass / (double)(sampleRate * channels)
                    );

                    if (sourceDurationToReadForFeed > TimeSpan.Zero)
                    {
                        // Read raw source samples into the feed buffer.
                        int rawSamplesRead = ReadAndReverseSourceSamples(
            ...
            var inputForWsolaSpan = _wsolaFeedBuffer.AsSpan(
                _wsolaFeedBufferReadOffset,
            ...
            int samplesWrittenToWsolaOut,
                samplesConsumedFromFeed,
                sourceSamplesRepresentedThisWsolaCall;
            bool endOfCurrentSourcePassFeed =
                _sourceSamplesFedToWsolaThisSourcePass + inputForWsolaSpan.Length >= sourceSamplesInOneSourcePass;
            if (
                inputForWsolaSpan.IsEmpty
                && (!IsSegmentLoopEffectivelyInfinite(_currentSourcePassBeingFedToWsola) || endOfCurrentSourcePassFeed)
            )
                break;

            // Handle flushing WSOLA if nearing the end of the source and not looping infinitely.
            if (
                endOfCurrentSourcePassFeed
            ...
            else
            {
                ...
                samplesWrittenToWsolaOut = _segmentWsolaStretcher.Process(
                    inputForWsolaSpan,
                    outputSpaceForWsolaSpan,
                    out samplesConsumedFromFeed,
                    out sourceSamplesRepresentedThisWsolaCall
                );
            }
            ...
            if (samplesWrittenToWsolaOut > 0)
                _wsolaOutputBufferValidSamples += samplesWrittenToWsolaOut;

            // Break if no progress is made or enough output is generated.
            if (samplesWrittenToWsolaOut == 0 && samplesConsumedFromFeed == 0)
                break;
            if (
                _wsolaOutputBufferValidSamples - _wsolaOutputBufferReadOffset
                >= WsolaTimeStretcher.DEFAULT_WINDOW_SIZE_FRAMES * channels
    ...
    {
        if (_wsolaFeedBufferReadOffset <= 0)
            return;
        if (_wsolaFeedBufferValidSamples > _wsolaFeedBufferReadOffset)
        {
            ...
            Buffer.BlockCopy(
                _wsolaFeedBuffer,
                _wsolaFeedBufferReadOffset * sizeof(float),
                _wsolaFeedBuffer,
                0,
    ...
    /// <returns>The number of samples written to the output buffer.</returns>
    private int ReadAndReverseSourceSamples(
        TimeSpan timeOffsetInCurrentSourcePass,
        TimeSpan durationToRead,
        Span<float> outputBuffer,
        int sampleRate,
        int channels,
        int currentSegmentLoopPassContext,
        bool isFeedingWsola = false
    )
    {
        int samplesToReadTotal = (int)(durationToRead.TotalSeconds * sampleRate * channels);
        samplesToReadTotal = Math.Min(samplesToReadTotal, outputBuffer.Length);
        if (samplesToReadTotal <= 0)
            return 0;

        int samplesWrittenToOutput = 0;
        long singlePassSourceSamples = (long)(SourceDuration.TotalSeconds * sampleRate * channels);
        if (singlePassSourceSamples <= 0)
            return 0;

        // Calculate the current sample offset within the source pass for the current read request.
        long currentEffectiveSampleOffsetInSourcePass = (long)(
            timeOffsetInCurrentSourcePass.TotalSeconds * sampleRate * channels
        );

        if (Settings.IsReversed)
        {
            // Ensure reversed buffer for the current loop iteration is populated
            int cacheKeySourceLoopPass = isFeedingWsola
                ? _currentSourcePassBeingFedToWsola
                : currentSegmentLoopPassContext;
            if (_reversedBufferCache == null || _reversedBufferCacheSourceLoopPass != cacheKeySourceLoopPass)
            {
                ...
                long physicalReadStart = (long)(SourceStartTime.TotalSeconds * sampleRate * channels);
                ...
                try
                {
                    ...
                    {
                        int toReadNow = Math.Min((int)singlePassSourceSamples - cachedSamples, tempChunk.Length);
                        int r = SourceDataProvider.ReadBytes(tempChunk.AsSpan(0, toReadNow));
                        if (r == 0)
                            break;
                ...
                if (cachedSamples < singlePassSourceSamples)
                    Array.Resize(ref _reversedBufferCache, cachedSamples);
        ...
        {
            long physicalProviderTargetReadPos = _currentSourceDataProviderPhysicalReadPos;
            ...
            long samplesLeftInThisSourcePass =
                singlePassSourceSamples
                - (physicalProviderTargetReadPos - (long)(SourceStartTime.TotalSeconds * sampleRate * channels));
            int samplesToReadThisIteration = Math.Min(
                samplesToReadTotal - samplesWrittenToOutput,
                (int)samplesLeftInThisSourcePass
            );
            ...
            if (samplesToReadThisIteration <= 0)
            {
                ...
                if (
                    IsSegmentLoopEffectivelyInfinite(
                        isFeedingWsola ? _currentSourcePassBeingFedToWsola : currentSegmentLoopPassContext
                    )
                )
                {
                    _currentSourceDataProviderPhysicalReadPos = (long)(
                        SourceStartTime.TotalSeconds * sampleRate * channels
                    );
                    if (isFeedingWsola)
                        _currentSourcePassBeingFedToWsola++;
                    continue;
            ...
            int readCount = SourceDataProvider.ReadBytes(
                outputBuffer.Slice(samplesWrittenToOutput, samplesToReadThisIteration)
            );
    ...
    private bool IsSegmentLoopEffectivelyInfinite(int currentSegmentLoopPassNumberBeingProcessed)
    {
        ...
        if (singleStretchedInstanceDuration <= TimeSpan.Zero)
            return false;
        int maxLoopsByDuration = (int)
            Math.Floor(Settings.Loop.TargetDuration.Value.TotalSeconds / singleStretchedInstanceDuration.TotalSeconds);
    ...
    {
        if (channels == 0 || buffer.Length == 0 || buffer.Length % channels != 0)
            return;
        int frameCount = buffer.Length / channels;
        float[] tempFrame = ArrayPool<float>.Shared.Rent(channels);
        try
        {
            for (int i = 0; i < frameCount / 2; i++)
            {
                int frameStartIndex = i * channels;
                int frameEndIndex = (frameCount - 1 - i) * channels;
    ...
    private static void SimpleResample(ReadOnlySpan<float> input, Span<float> output, int channels)
    {
        ...
        double ratio = outputFrames == 1 || inputFrames == 1 ? 0 : (double)(inputFrames - 1) / (outputFrames - 1);
        for (int i = 0; i < outputFrames; i++)
        {
            double inputFrameDouble = i * ratio;
            int inputFrameFloor = (int)inputFrameDouble;
            double fraction = inputFrameDouble - inputFrameFloor;
            inputFrameFloor = Math.Clamp(inputFrameFloor, 0, inputFrames - 1);
            int inputFrameCeil = Math.Min(inputFrameFloor + 1, inputFrames - 1);
            for (int ch = 0; ch < channels; ch++)
            {
                // Linear interpolation
                float s0 = input[inputFrameFloor * channels + ch];
                float s1 = input[inputFrameCeil * channels + ch];
    ...
    private static float GetFadeMultiplier(double progress, FadeCurveType curve, bool isFadingOutEffect)
    {
        ...
        float rawMultiplier = curve switch
        {
