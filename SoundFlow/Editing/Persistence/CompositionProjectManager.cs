...
{
    private const string CURRENT_PROJECT_FILE_VERSION = "1.0.5";
    private const string CONSOLIDATED_MEDIA_FOLDER_NAME = "Assets";
    private const long MAX_EMBED_SIZE_BYTES = 1 * 1024 * 1024; // 1MB threshold for embedding
    ...
    )
    {
        ...
        {
            ProjectFileVersion = CURRENT_PROJECT_FILE_VERSION,
        ...
        string projectDirectory =
            Path.GetDirectoryName(projectFilePath) ?? throw new IOException("Invalid project file path.");
        string mediaAssetsDirectory = Path.Combine(projectDirectory, CONSOLIDATED_MEDIA_FOLDER_NAME);

        if (consolidateMedia)
            Directory.CreateDirectory(mediaAssetsDirectory);
        ...
        string json = JsonSerializer.Serialize(projectData, _serializerOptions);
    ...
    )
    {
        ...
        if (
            embedSmallMedia
            && provider is { CanSeek: true, Length: > 0 }
            && (long)provider.Length * provider.SampleFormat.GetBytesPerSample() < MAX_EMBED_SIZE_BYTES
        )
        {
            int samplesToEmbed = provider.Length;
            if (samplesToEmbed > 0)
            {
                float[] tempBuffer = ArrayPool<float>.Shared.Rent(samplesToEmbed);
                try
                {
                    provider.Seek(0);
                    int readCount = provider.ReadBytes(tempBuffer.AsSpan(0, samplesToEmbed));
                    if (readCount == samplesToEmbed)
                    {
                        byte[] byteBuffer = new byte[readCount * sizeof(float)];
        ...
        {
            string consolidatedFileName = $"{sourceRef.Id:N}.wav";
            string consolidatedFilePath = Path.Combine(mediaAssetsDirectory, consolidatedFileName);

            // Check if this exact source (by ID) has already been consolidated.
            if (!File.Exists(consolidatedFilePath))
            {
                int totalSamples = provider.Length;
                float[] tempBuffer = ArrayPool<float>.Shared.Rent(totalSamples);
                try
                {
                    provider.Seek(0); // Read from the beginning
                    int samplesRead = provider.ReadBytes(tempBuffer.AsSpan(0, totalSamples));
                    ...
                    else if (totalSamples == 0)
                    {
                        ...
                        await File.WriteAllBytesAsync(
                            consolidatedFilePath,
                            CreateEmptyWavHeader(compositionSampleRate, compositionTargetChannels, SampleFormat.F32)
                        );
                    }
                    else
                    {
                        Console.WriteLine(
                            $"Warning: Could not read all samples from in-memory provider for consolidation (ID: {sourceRef.Id}). Expected {totalSamples}, got {samplesRead}."
                        );
    ...
    public static async Task<(
        Composition Composition,
        List<ProjectSourceReference> UnresolvedSources
    )> LoadProjectAsync(string projectFilePath)
    {
        ...
        var projectData =
            JsonSerializer.Deserialize<ProjectData>(json, _serializerOptions)
            ?? throw new JsonException("Failed to deserialize project data.");

        if (
            Version.TryParse(projectData.ProjectFileVersion, out var fileVersion)
            && Version.TryParse(CURRENT_PROJECT_FILE_VERSION, out var currentVersion)
            && fileVersion.Major > currentVersion.Major
        )
        {
            Console.WriteLine(
                $"Warning: Loading project file version {projectData.ProjectFileVersion} "
                    + $"with library version {CURRENT_PROJECT_FILE_VERSION}. Forward compatibility is not guaranteed."
            );
        }
        ...
        foreach (var projectTrack in projectData.Tracks)
        {
            ...
            {
                var sourceRef = projectData.SourceReferences.FirstOrDefault(sr =>
                    sr.Id == projectSegment.SourceReferenceId
                );
                ...
                {
                    Console.WriteLine(
                        $"Warning: Audio source for segment '{projectSegment.Name}' (Ref ID: {projectSegment.SourceReferenceId}) is missing. Using placeholder."
                    );
                    var silentDuration = projectSegment.SourceDuration;
                    int placeholderSampleCount = (int)(
                        Math.Max(silentDuration.TotalSeconds, 0.1) * composition.SampleRate * composition.TargetChannels
                    );
    ...
    private static ISoundDataProvider? ResolveSourceReferenceAsync(
        ProjectSourceReference sourceRef,
        string projectDirectory
    )
    {
        // 1. Try Embedded
        if (sourceRef.IsEmbedded && !string.IsNullOrEmpty(sourceRef.EmbeddedDataB64))
        {
            try
            {
                byte[] byteBuffer = Convert.FromBase64String(sourceRef.EmbeddedDataB64);
                // Embedded data is saved as F32 float array samples
                if (sourceRef.OriginalSampleFormat == SampleFormat.F32)
                {
                    float[] floatArray = new float[byteBuffer.Length / sizeof(float)];
                    Buffer.BlockCopy(byteBuffer, 0, floatArray, 0, byteBuffer.Length);
                    return new RawDataProvider(floatArray);
                }
                Console.WriteLine(
                    $"Warning: Unsupported embedded format {sourceRef.OriginalSampleFormat} for source ID {sourceRef.Id}"
                );
                return null;
            }
            catch (Exception ex)
        ...
        if (!string.IsNullOrEmpty(sourceRef.ConsolidatedRelativePath))
        {
            ...
            Console.WriteLine(
                $"Warning: Consolidated file not found for source ID {sourceRef.Id} at expected path: {pathToTry}"
            );
        }
        ...
        if (!string.IsNullOrEmpty(sourceRef.OriginalAbsolutePath))
        {
            ...
            Console.WriteLine(
                $"Warning: Original absolute file not found for source ID {sourceRef.Id} at path: {pathToTry}"
            );
        }
        ...
        where T : class
    {
        ...
        foreach (var effect in effects)
        {
            ...
            {
                if (prop is not { CanRead: true, CanWrite: true } || prop.GetIndexParameters().Length != 0)
                    continue;
                if (
                    prop.DeclaringType == typeof(SoundComponent)
                    || prop.DeclaringType == typeof(SoundModifier)
                    || prop.DeclaringType == typeof(AudioAnalyzer) && prop.Name is "Name" or "Enabled"
                )
                {
                    parameters[prop.Name] = prop.Name switch
                    {
                        "Enabled" when effect is SoundModifier or AudioAnalyzer => JsonValue.Create(
                            effect switch
                            {
                        ...
                        "Name" when effect is SoundModifier or AudioAnalyzer => JsonValue.Create(
                            effect switch
                            {
                ...
                if (prop.Name is "ParentSegment" or "ParentTrack" or "ParentComposition")
                    continue;

                try
                {
                    object? value = prop.GetValue(effect);
                    if (value != null)
                        parameters[prop.Name] = JsonValue.Create(
                            JsonSerializer.SerializeToElement(value, _serializerOptions)
                        );
                }
                catch (Exception ex)
                {
                    Console.WriteLine(
                        $"Warning: Could not serialize property '{prop.Name}' for effect type '{effectType.Name}': {ex.Message}"
                    );
                }
            }

            effectDataList.Add(
                new ProjectEffectData
                {
                    ...
                    Parameters = JsonDocument.Parse(parameters.ToJsonString(_serializerOptions)),
                }
            );
        }
    ...
    private static List<T> DeserializeEffects<T>(List<ProjectEffectData> effectDataList)
        where T : class
    {
        ...
        foreach (var effectData in effectDataList)
        {
            ...
            {
                Console.WriteLine(
                    $"Warning: Could not find effect type '{effectData.TypeName}'. Effect will be skipped."
                );
                continue;
            }
            ...
            {
                Console.WriteLine(
                    $"Warning: Type '{effectData.TypeName}' is not assignable to target type '{typeof(T).Name}'. Skipping."
                );
                continue;
            }
            ...
            try
            {
                ...
                {
                    Console.WriteLine(
                        $"Warning: Could not create instance of effect type '{effectData.TypeName}'. Skipping."
                    );
                    continue;
                }
                ...
                if (effectData.Parameters != null)
                {
                    ...
                    foreach (var propInfo in effectType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
                    {
                        if (propInfo.CanWrite && parametersNode.TryGetProperty(propInfo.Name, out var jsonProp))
                        {
                            ...
                            {
                                Console.WriteLine(
                                    $"Warning: Could not deserialize or set property '{propInfo.Name}' for effect '{effectType.Name}': {ex.Message}. Using default."
                                );
                            }
                        }
                        else if (propInfo is { CanWrite: true, Name: "Enabled" }) // Handle Enabled if not in JSON explicitly
                        {
                            switch (effectInstance)
                            {
                                case SoundModifier sm:
                                    sm.Enabled =
                                        parametersNode.TryGetProperty("IsEnabled", out var enabledJsonProp1)
                                ...
                                case AudioAnalyzer aa:
                                    aa.Enabled =
                                        parametersNode.TryGetProperty("IsEnabled", out var enabledJsonProp2)
