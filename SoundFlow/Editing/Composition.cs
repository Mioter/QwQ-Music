...
public class Composition : ISoundDataProvider
{
    ...
    public float MasterVolume
    {
        ...
        set
        {
            if (!(Math.Abs(_masterVolume - value) > 0.0001f))
                return;
            _masterVolume = value;
            MarkDirty();
        }
    ...
    public int TargetChannels
    {
        ...
        {
            if (value <= 0)
                throw new ArgumentOutOfRangeException(nameof(value), "Channels must be greater than 0.");
            if (_targetChannels == value)
                return;
    ...
    {
        int samplesToRender = (int)(duration.TotalSeconds * SampleRate * TargetChannels);
        if (samplesToRender <= 0)
            return [];

        float[] outputBuffer = new float[samplesToRender];
    ...
    {
        var soloedTracks = Tracks
            .Where(t => t.Settings is { IsSoloed: true, IsEnabled: true, IsMuted: false })
            .ToList();
        return soloedTracks.Count != 0
            ? soloedTracks
            : Tracks.Where(t => t.Settings is { IsMuted: false, IsEnabled: true }).ToList();
    }
    ...
    public int ReadBytes(Span<float> buffer)
    {
        if (IsDisposed)
            return 0;
        var currentTime = TimeSpan.FromSeconds((double)Position / SampleRate / TargetChannels);
        ...
        int samplesWritten = Render(currentTime, durationToRead, buffer);
    ...
    public void Seek(int sampleOffset)
    {
        ...
        Position = Math.Clamp(sampleOffset, 0, Length);
        PositionChanged?.Invoke(this, new PositionChangedEventArgs(Position));
    }
    ...
    public bool ReplaceSegment(
        Track track,
        TimeSpan originalStartTime,
        TimeSpan originalEndTime,
        ISoundDataProvider replacementSource,
        TimeSpan replacementSourceStartTime,
        TimeSpan replacementSourceDuration
    )
    {
        var segmentToReplace = track.Segments.FirstOrDefault(s =>
            s.TimelineStartTime == originalStartTime && s.TimelineEndTime == originalEndTime
        );
        if (segmentToReplace == null)
            return false;
    ...
    {
        bool removed = track.RemoveSegment(segmentToRemove, shiftFollowing);
        if (removed)
            MarkDirty();
        return removed;
    }
    ...
    {
        var segment = track.Segments.FirstOrDefault(s =>
            s.TimelineStartTime == startTime && s.TimelineEndTime == endTime
        );
        bool removed = segment != null && track.RemoveSegment(segment, shiftFollowing);
        if (removed)
            MarkDirty();
        return removed;
    }
    ...
    public AudioSegment? SilenceSegment(Track track, TimeSpan rangeStartTime, TimeSpan rangeDuration)
    {
        ...
        int silentSamplesCount = (int)(rangeDuration.TotalSeconds * SampleRate * TargetChannels);
        ...
        for (int i = segmentsOnTrack.Count - 1; i >= 0; i--)
        {
            ...
            if (segmentTimelineStart < rangeStartTime && segmentTimelineEnd > rangeEndTime)
            {
                ...
                var part1SourceDuration = TimeSpan.FromTicks(
                    (long)(part1TimelineDuration.Ticks * segment.Settings.SpeedFactor)
                );
            ...
            if (
                segmentTimelineStart < rangeStartTime
                && segmentTimelineEnd > rangeStartTime /* implies segmentTimelineEnd <= rangeEndTime */
            )
            {
                var newTimelineDuration = rangeStartTime - segmentTimelineStart;
                var newSourceDuration = TimeSpan.FromTicks(
                    (long)(newTimelineDuration.Ticks * segment.Settings.SpeedFactor)
                );
            ...
            if (segmentTimelineStart >= rangeStartTime && segmentTimelineStart < rangeEndTime)
            {
                ...
                var sourceTimeShiftAmount = TimeSpan.FromTicks(
                    (long)(timelineShiftAmount.Ticks * segment.Settings.SpeedFactor)
                );
    ...
    public void InsertSegment(
        Track track,
        TimeSpan insertionPoint,
        AudioSegment segmentToInsert,
    ...
    {
        bool removed = Analyzers.Remove(analyzer);
        if (removed)
            MarkDirty();
        return removed;
    }
