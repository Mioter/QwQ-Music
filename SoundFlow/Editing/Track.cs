...
public class Track
{
    ...
    public bool RemoveSegment(AudioSegment segment, bool shiftSubsequent = false)
    {
        ...
        bool removed = Segments.Remove(segment);
        if (removed)
        {
            if (shiftSubsequent)
            {
    ...
    )
    {
        ...
        int totalSamplesToRender = (int)(durationToRender.TotalSeconds * targetSampleRate * targetChannels);
        ...
        try
        {
            foreach (var segment in Segments)
            {
                ...
                if (overlapStart < overlapEnd) // There is an overlap
                {
                    ...
                    int samplesInOverlap = (int)(durationOfOverlap.TotalSeconds * targetSampleRate * targetChannels);
                    if (samplesInOverlap <= 0)
                        continue;

                    // Allocate/resize segment output buffer if needed
                    if (segmentOutputBuffer == null || segmentOutputBuffer.Length < samplesInOverlap)
                    {
                        if (segmentOutputBuffer != null)
                            ArrayPool<float>.Shared.Return(segmentOutputBuffer);
                    ...
                    segment.ReadProcessedSamples(
                        segmentTimeOffsetForRead,
                        durationOfOverlap,
                        segmentOutputSpan,
                        0,
                        targetSampleRate,
                    ...
                    int outputBufferStartOffset = (int)(
                        (overlapStart - overallStartTime).TotalSeconds * targetSampleRate * targetChannels
                    );

                    for (int i = 0; i < samplesInOverlap; i++)
                    {
        ...
        for (int i = 0; i < totalSamplesToRender; i++)
        {
