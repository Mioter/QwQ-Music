...
public class AudioSegmentSettings
{
    ...
    public float Pan
    {
        ...
        set
        {
            if (Math.Abs(_pan - value) < float.Epsilon)
                return;
            _pan = value;
            MarkDirty();
        }
    ...
    public float SpeedFactor
    {
        ...
        set
        {
            if (Math.Abs(_speedFactor - value) < float.Epsilon)
                return;
            _speedFactor = value;
            MarkDirty();
        }
    ...
    {
        get =>
            _targetStretchDuration.HasValue && ParentSegment != null && ParentSegment.SourceDuration > TimeSpan.Zero
                ? (float)(_targetStretchDuration.Value.TotalSeconds / ParentSegment.SourceDuration.TotalSeconds)
                : _timeStretchFactor;
        set
        {
            if (value <= 0)
                throw new ArgumentOutOfRangeException(nameof(value), "TimeStretchFactor must be greater than zero.");
            if (!(Math.Abs(_timeStretchFactor - value) > float.Epsilon) && !_targetStretchDuration.HasValue)
                return;
    ...
    public TimeSpan? TargetStretchDuration
    {
        ...
        set
        {
            if (value.HasValue && value.Value <= TimeSpan.Zero)
                throw new ArgumentOutOfRangeException(nameof(value), "TargetStretchDuration must be positive if set.");
            if (_targetStretchDuration == value)
                return;
