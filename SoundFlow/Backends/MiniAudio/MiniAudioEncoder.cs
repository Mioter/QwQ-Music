...
internal sealed unsafe class MiniAudioEncoder : ISoundEncoder
{
    ...
    )
    {
        ...
        IntPtr config = Native.AllocateEncoderConfig(encodingFormat, sampleFormat, (uint)channels, (uint)sampleRate);

        // Allocate encoder and initialize
        _encoder = Native.AllocateEncoder();
        var result = Native.EncoderInit(
    ...
    public int Encode(Span<float> samples)
    {
        lock (_syncLock)
        {
            ...
            ulong framesToWrite = (ulong)(samples.Length / AudioEngine.Channels);
