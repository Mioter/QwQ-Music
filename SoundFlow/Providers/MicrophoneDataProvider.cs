...
public class MicrophoneDataProvider : ISoundDataProvider, IDisposable
{
    ...
    public void StopCapture()
    {
        ...
        if (_currentBuffer != null && _currentBufferIndex > 0)
        {
            ...
            float[] remainingBuffer = new float[_currentBufferIndex];
    ...
    private void EnqueueAudioData(Span<float> samples, Capability capability)
    {
        ...
        while (samplesRemaining > 0)
        {
            ...
            int spaceLeftInBuffer = _bufferSize - _currentBufferIndex;
            int samplesToCopy = Math.Min(samplesRemaining, spaceLeftInBuffer);
    ...
    public int ReadBytes(Span<float> buffer)
    {
        ...
        while (bytesCopied < buffer.Length && _bufferQueue.TryDequeue(out float[]? audioData))
        {
            int remainingBufferSpace = buffer.Length - bytesCopied;
            int bytesToCopy = Math.Min(remainingBufferSpace, audioData.Length);
            ...
            {
                float[] remainingAudioData = audioData.AsSpan(bytesToCopy).ToArray();
