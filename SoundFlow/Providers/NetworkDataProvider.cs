...
public sealed class NetworkDataProvider : ISoundDataProvider, IDisposable
{
            ...
            try
            {
                ...
                if (response.Content.Headers.ContentLength is > 0)
                {
                    try
                    {
                        ...
                        byte[] buffer = new byte[partialContentStream.Length];
    ...
    private void ParseHlsPlaylist(string playlistContent, string baseUrl)
    {
        ...
        foreach (string? line in lines)
        {
            ...
            {
                string durationStr = trimmedLine["#EXT-X-TARGETDURATION:".Length..];
            ...
            {
                string durationStr = trimmedLine["#EXTINF:".Length..].Split(',')[0];
                if (double.TryParse(durationStr, out double duration))
