...
public class RawDataProvider : ISoundDataProvider
{
        ...
        SampleFormat =
            sampleFormat != SampleFormat.Unknown
                ...
                : throw new ArgumentException(
                    "SampleFormat cannot be Unknown for RawDataProvider when using a stream.",
                    nameof(sampleFormat)
                );
    }
    ...
    public RawDataProvider(byte[] rawBytes, SampleFormat sampleFormat)
    {
        ...
        SampleFormat =
            sampleFormat != SampleFormat.Unknown
                ? sampleFormat
                : throw new ArgumentException(
                    "SampleFormat cannot be Unknown for RawDataProvider when using a byte array.",
                    nameof(sampleFormat)
                );
    }
    ...
    public int ReadBytes(Span<float> buffer)
    {
        ...
        int samplesActuallyRead = ReadData(buffer);
    ...
    public void Seek(int sampleOffset)
    {
        ...
        if (_pcmStream != null)
        {
            ...
            sampleOffset = ClampSampleOffset(sampleOffset, _pcmStream.Length / SampleFormat.GetBytesPerSample());
            _pcmStream.Seek((long)sampleOffset * SampleFormat.GetBytesPerSample(), SeekOrigin.Begin);
        }
        else
        {
    ...
    private int GetLength()
    {
        if (_pcmStream != null)
        {
            return _pcmStream.CanSeek ? (int)(_pcmStream.Length / SampleFormat.GetBytesPerSample()) : -1;
        }
    ...
    private int GetArrayLength()
    {
        return _floatData?.Length
            ?? _byteArray?.Length / SampleFormat.GetBytesPerSample()
            ?? _intArray?.Length
            ?? _shortData?.Length
            ?? 0;
    }
    ...
    private int ReadData(Span<float> buffer)
    {
        ...
        if (_byteArray != null)
        {
            ...
            int byteOffset = Position * bytesPerSample;
            int bytesToRead = Math.Min(buffer.Length * bytesPerSample, _byteArray.Length - byteOffset);

            if (bytesToRead <= 0)
                return 0;

            ConvertBytesToFloat(
                _byteArray.AsSpan(byteOffset, bytesToRead),
                buffer[..(bytesToRead / bytesPerSample)],
                SampleFormat
            );
    ...
    {
        int bytesPerSample = SampleFormat.GetBytesPerSample();
        int bytesToRead = buffer.Length * bytesPerSample;
    ...
    {
        int remainingSamples = source.Length - Position;
        int samplesActuallyRead = Math.Min(buffer.Length, remainingSamples);
        ...
        {
            buffer[i] = convertAction(source[Position + i]);
        }
    ...
    private static void ConvertBytesToFloat(Span<byte> byteBuffer, Span<float> floatBuffer, SampleFormat format)
    {
        ...
        switch (format)
        {
            ...
            case SampleFormat.S24:
                ...
                {
                    int byteIndex = i * 3;
                    floatBuffer[i] = byteIndex + 2 < byteBuffer.Length ? Convert24BitToFloat(byteBuffer, byteIndex) : 0;
                }
                break;
    ...
    {
        int sample24 = bytes[index] << 0 | bytes[index + 1] << 8 | bytes[index + 2] << 16;
