...
///     实现类需要负责管理数据库连接和事务处理
/// </remarks>
public interface IDatabaseRepository<T> : IAsyncDisposable
{
    /// <summary>
    ///     根据主键值获取单个实体对象
    /// </summary>
    ...
    ///     适用于部分字段更新的场景，提高性能
    /// </remarks>
    public Task UpdateAsync(string primaryKey, string[] fields, string?[] values);
    ...
    public Task UpdateAsync(string primaryKey, Dictionary<string, object?> fieldValues);
    ...
    public Task DeleteAsync(string primaryKey);
    ...
    public Task<bool> ExistsAsync(string primaryKey);
}
