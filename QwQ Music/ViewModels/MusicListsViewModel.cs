...
public partial class MusicListsViewModel : ObservableObject
{
    private static readonly Lazy<MusicListsViewModel> _instance = new(() => new MusicListsViewModel());
    public static MusicListsViewModel Instance => _instance.Value;

    private MusicListsViewModel() { }
    ...
    private async Task<CreateMusicListViewModel?> CreateMusicList()
    {
        ...
        try
        {
            MusicListModel? musicListItem;
            if (model.Cover != null)
            {
                string coverFilePath = Path.Combine(MainConfig.PlaylistCoverSavePath, $"{model.Name}.png");
                musicListItem = new MusicListModel(model.Name, model.Description, "", coverFilePath, model.Cover);
                await FileOperation.SaveImageAsync(model.Cover, coverFilePath);
            }
            else
            {
                musicListItem = new MusicListModel(model.Name, model.Description);
            }
    ...
    private async Task DeleteMusicListModel(MusicListModel model)
    {
        ...
        try
        {
            ...
            MainWindowViewModel.Instance.RemoveTabPage(model.Key);
    ...
    public void SynchronizeMusicDeletion(MusicItemModel item)
    {
        // 从所有已加载的歌单中移除该音乐
