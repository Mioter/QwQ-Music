...
public partial class LyricsModel(LyricsData lyricsData) : ObservableObject
{
    ...
    public int LyricsIndex
    {
        ...
        set
        {
            if (_lyricsIndex == value || value < 0 || value >= Count) 
                return;

            _lyricsIndex = value;
            MusicPlayerViewModel.Instance.CurrentDurationInSeconds = _timePoints[_lyricsIndex];
        }
    }
    ...
    {
        int lyricsIndex = CalculateIndex(timePoints);
        if (_lyricsIndex == lyricsIndex || lyricsIndex < 0 || lyricsIndex >= Count) 
            return;
        
        _lyricsIndex = lyricsIndex;
        OnPropertyChanged(nameof(LyricsIndex));
